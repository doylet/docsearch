...
impl StandardConfig {
    ...
    pub fn validate(&self) -> Result<(), ConfigValidationError> {
        ...
        if self.server_host.is_empty() {
            return Err(ConfigValidationError::InvalidHost(
                "Host cannot be empty".to_string(),
            ));
        }

        if self.collection_name.is_empty() {
            return Err(ConfigValidationError::InvalidCollection(
...
pub mod helpers {
    ...
    pub fn validate_compatibility(
        cli_config: &StandardConfig,
        server_config: &StandardConfig,
    ) -> Result<(), String> {
        if cli_config.server_port != server_config.server_port {
            return Err(format!(
                "Port mismatch: CLI expects {}, Server uses {}",
                cli_config.server_port, server_config.server_port
            ));
        }
        ...
        if cli_config.collection_name != server_config.collection_name {
            return Err(format!(
                ...
                cli_config.collection_name, server_config.collection_name
            ));
        }
