...
pub use monitoring::{ProductionMonitor, SystemMetrics, ServiceMetrics};
pub use shutdown::{GracefulShutdown, ShutdownSignal};
pub use startup::{StartupValidator, StartupResult, ValidationStep};
...
impl ProductionDeployment {
    ...
    pub async fn initialize(&mut self) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
        ...
        if self.config.monitoring_enabled {
            let monitoring_config = crate::infrastructure::production::monitoring::MonitoringConfig {
                ...
                alert_thresholds: crate::infrastructure::production::monitoring::AlertThresholds {
    ...
    pub async fn start(&mut self) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
        ...
        if let Some(validator) = &mut self.startup_validator {
            ...
            if !validation_result.success {
                return Err(format!("Startup validation failed: {:?}", validation_result.critical_failures).into());
            }
    ...
    pub async fn shutdown(&mut self) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
        ...
        if let Some(shutdown_handler) = &mut self.shutdown_handler {
            ...
            shutdown_handler.initiate_shutdown(ShutdownSignal::Graceful).await?;
        }
