...
impl SimpleQueryEnhancer {
    ...
    fn analyze_query_context(&self, query: &str) -> QueryContext {
        ...
        let context = QueryContext {
            is_question: query.contains("?") || query_lower.starts_with("how") || query_lower.starts_with("what") || query_lower.starts_with("why"),
            is_troubleshooting: query_lower.contains("error") || query_lower.contains("issue") || query_lower.contains("problem") || query_lower.contains("fix"),
            is_tutorial: query_lower.contains("tutorial") || query_lower.contains("guide") || query_lower.contains("how to"),
            is_setup: query_lower.contains("setup") || query_lower.contains("install") || query_lower.contains("configure"),
    ...
    fn calculate_complexity_score(&self, query: &str) -> f32 {
        ...
        let has_technical_terms = query.split_whitespace().any(|word| {
            self.technical_terms.contains_key(&word.to_lowercase())
        });
    ...
    fn expand_with_context(&self, query: &str, context: &QueryContext) -> Vec<String> {
        ...
        if context.is_troubleshooting {
            expansions.extend(vec!["debug".to_string(), "fix".to_string(), "resolve".to_string()]);
        }
        ...
        if context.is_setup {
            expansions.extend(vec!["installation".to_string(), "getting started".to_string()]);
        }
    ...
    fn extract_synonyms_from_expansion(&self, expansions: &[String]) -> Vec<String> {
        ...
        expansions.iter()
            .filter(|term| {
                ...
                self.synonyms.values().any(|synonyms| synonyms.contains(term))
            })
            .cloned()
            .collect()
    }
...
impl ResultRanker for MultiFactorResultRanker {
    async fn rank(&self, results: Vec<SearchResult>) -> Result<Vec<SearchResult>> {
        ...
        for result in results {
            let signals = self.calculate_comprehensive_score(&result, &query_terms).await;
        ...
        let ranked_results: Vec<SearchResult> = scored_results
            ...
            .map(|(mut result, signals)| {
                ...
                result.final_score = zero_latency_core::values::Score::new(signals.final_combined_score)
        ...
        tracing::info!(
            ...
            ranked_results.first().map(|r| r.final_score.value()).unwrap_or(0.0)
        );
    ...
    async fn explain_ranking(&self, result: &SearchResult) -> Result<RankingSignals> {
        ...
        Ok(RankingSignals {
            ...
            term_frequency: zero_latency_core::values::Score::new(signals.keyword_density).unwrap_or_default(),
            document_frequency: zero_latency_core::values::Score::new(signals.content_relevance_score).unwrap_or_default(),
            title_boost: signals.title_boost_score,
            freshness_boost: signals.recency_score,
            custom_signals: {
                let mut map = HashMap::new();
                map.insert("metadata_relevance".to_string(), signals.metadata_relevance_score);
                map.insert("exact_match_bonus".to_string(), signals.exact_match_bonus);
                map.insert("final_combined_score".to_string(), signals.final_combined_score);
                map
            },
        })
    }
...
impl MultiFactorResultRanker {
    ...
    fn calculate_content_relevance_score(&self, result: &SearchResult, query_terms: &[String]) -> f32 {
        ...
        for term in query_terms {
            ...
            let matches = words.iter().filter(|word| word.contains(&term_lower)).count();
            if matches > 0 {
                matched_terms += 1;
    ...
    fn calculate_title_boost_score(&self, result: &SearchResult) -> f32 {
        ...
        let uppercase_ratio = content.chars().filter(|c| c.is_uppercase()).count() as f32 / content.chars().count() as f32;
    ...
    fn calculate_metadata_relevance_score(&self, result: &SearchResult, query_terms: &[String]) -> f32 {
        if query_terms.is_empty() {
            return 0.5;
        }
        ...
        if !result.heading_path.is_empty() {
            ...
            let heading_matches = query_terms.iter()
        ...
        let path_matches = query_terms.iter()
        ...
        if let Some(ref url) = result.url {
            ...
            let url_matches = query_terms.iter()
    ...
    fn calculate_keyword_density(&self, result: &SearchResult, query_terms: &[String]) -> f32 {
        ...
        let total_keyword_matches = query_terms.iter()
            .map(|term| {
                let term_lower = term.to_lowercase();
                words.iter().filter(|word| word.contains(&term_lower)).count()
            })
            .sum::<usize>();
    ...
    fn calculate_exact_match_bonus(&self, result: &SearchResult, query_terms: &[String]) -> f32 {
        ...
        let exact_matches = query_terms.iter()
            .filter(|term| {
                let term_lower = term.to_lowercase();
                content_lower.split_whitespace().any(|word| word == term_lower)
            })
            .count();
    ...
    fn extract_common_query_terms(&self, results: &[SearchResult]) -> Vec<String> {
        ...
        for result in results.iter().take(5) { // Analyze top 5 results
            let content_lower = result.content.to_lowercase();
