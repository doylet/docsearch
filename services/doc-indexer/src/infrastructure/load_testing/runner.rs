...
use crate::infrastructure::load_testing::{
    ...
    scenario::{LoadTestScenario, ScenarioRequest, ScenarioResponse},
    LoadTestConfig,
};
...
impl LoadTestRunner {
    ...
    pub async fn run(
        ...
        let memory_monitor = self
            .start_memory_monitoring(metrics_collector.clone())
            .await;
        ...
        for handle in handles {
            match handle.await {
                Ok(result) => {
                    ...
                    match &result.result {
                        Ok(_) => metrics_collector
                            .record_success(&result.scenario_name, result.execution_time),
                        Err(error) => metrics_collector
                            .record_error(&result.scenario_name, &error.to_string()),
                    }
                    results.push(result);
                }
                Err(join_error) => {
    ...
    fn analyze_results(&self, metrics: &LoadTestMetrics) -> LoadTestAnalysis {
        ...
        if metrics.performance.response_times.mean > Duration::from_millis(1000) {
            ...
            recommendations.push(
                "Response times exceed 1s threshold - consider performance optimization"
                    .to_string(),
            );
        }
        ...
        if metrics.memory.leak_detection.suspected_leak {
            success = false;
            recommendations
                .push("Memory leak detected - investigate memory management".to_string());
        }

        if metrics.memory.process_memory.peak_mb > 1000.0 {
            recommendations
                .push("High memory usage detected - consider memory optimization".to_string());
        }

        // Error analysis
        if metrics.errors.error_rate > 0.01 {
        ...
        let summary =
            crate::infrastructure::load_testing::metrics::generate_summary_report(metrics);
    ...
    fn validate_memory_optimizations(&self, metrics: &LoadTestMetrics) -> Vec<String> {
        ...
        if metrics.memory.leak_detection.stability_score > 0.8 {
            ...
            recommendations
                .push("⚠️ Memory usage unstable - review optimization settings".to_string());
        }
