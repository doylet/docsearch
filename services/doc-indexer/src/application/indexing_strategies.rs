...
use zero_latency_core::{models::Document, Result, Uuid};
use zero_latency_vector::{VectorDocument, VectorMetadata};

use super::content_processing::ContentProcessor;
use super::interfaces::{EmbeddingService, VectorStorage};
    ...
    async fn index_document(
        ...
        for (chunk_index, chunk_content) in chunks.into_iter().enumerate() {
            let embedding = embedding_service
                .generate_embeddings(&chunk_content)
                .await?;
            ...
            metadata.insert(
                "path".to_string(),
                serde_json::Value::String(document.path.to_string_lossy().to_string()),
            );
            metadata.insert(
            ...
            metadata.insert(
                ...
                serde_json::Value::String(chunk_content.chars().take(200).collect::<String>()),
            );
...
impl IndexingStrategy for FastIndexingStrategy {
    async fn index_document(
        ...
        if document.content.len() <= self.config.chunk_size {
            ...
            let embedding = embedding_service
                .generate_embeddings(&document.content)
                .await?;
            ...
            metadata.insert(
                "strategy".to_string(),
                serde_json::Value::String("fast_single".to_string()),
            );
            ...
            let vector_doc = VectorDocument {
                ...
                metadata: VectorMetadata {
                    ...
                    url: None,
                    collection: Some(collection.to_string()),
                    custom: HashMap::from([("strategy".to_string(), "fast_single".to_string())]),
                },
            };
            ...
            while start < document.content.len()
                && chunk_index < self.config.max_chunks_per_document.unwrap_or(10)
            {
                ...
                if chunk.len() >= self.config.min_chunk_size {
                    ...
                    metadata.insert(
                        "strategy".to_string(),
                        serde_json::Value::String("fast_chunked".to_string()),
                    );
...
impl IndexingStrategy for PrecisionIndexingStrategy {
    async fn index_document(
        ...
        for (chunk_index, chunk_content) in chunks.into_iter().enumerate() {
            ...
            metadata.insert(
                "strategy".to_string(),
                serde_json::Value::String("precision".to_string()),
            );
            metadata.insert(
                "content_preview".to_string(),
                serde_json::Value::String(chunk_content.chars().take(200).collect::<String>()),
            ...
            let vector_doc = VectorDocument {
                ...
                metadata: VectorMetadata {
                    ...
                    url: None,
                    collection: Some(collection.to_string()),
                    custom: HashMap::from([("strategy".to_string(), "precision".to_string())]),
                },
            };
